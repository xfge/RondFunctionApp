...
const CITY_STATES: Record<string, { osmId: number; amapName?: string; aliases?: string[] }> = {
    HK: {
        osmId: 913110,
        amapName: "香港特别行政区",
        aliases: ["香港", "香港特别行政区", "香港特別行政區", "Hong Kong"],
    },
    MO: {
        osmId: 1867188,
        amapName: "澳门特别行政区",
        aliases: ["澳门", "澳門", "澳门特别行政区", "澳門特別行政區", "Macau", "Macao"],
    },
    SG: { osmId: 536780 },
    MC: { osmId: 1124039 },
    SM: { osmId: 54624 },
 ...
 *   - Special case: if the city name matches a city-state alias
 *     (e.g. "香港", "香港特别行政区", "澳门", "澳门特别行政区"),
 *     the client has mislabeled
...
export function resolveRoute(countryCode?: string, deviceRegion?: string, city?: string): RoutingResult {
    ...
    // Mislabeled city-state: country_code=CN but the city name is a city-state
