...
const COUNTRY_CITY_ADMIN_LEVEL: Record<string, number[]> = {
    ...
    CA: [6, 8], // 6 = regional municipality / county (Regional Municipality of Niagara,
                //     Northumberland County) used when the requested city name is not a
                //     valid OSM boundary; 8 = city / town (Toronto, Vancouver) as fallback.
...
function extractMatch(features: GeoapifyFeature[], city: string, area?: string, countryCode?: string): GeoapifyMatchResult | null {
    ...
    // 5. Fallback: most specific boundary (highest admin_level ≤ 8, skipping country and sub-city levels)
    const fallbackMatch = !anyNameMatch && !areaMatch && !countryMatch && !categoryMatch
        ? [...features]
              .filter((f) => {
                  const level = f.properties.datasource?.raw?.admin_level ?? 0;
                  return f.properties.datasource?.raw?.osm_id != null && level > 2 && level <= 8;
              })
              .sort((a, b) =>
                  (b.properties.datasource?.raw?.admin_level ?? 0) -
                  (a.properties.datasource?.raw?.admin_level ?? 0))
