...
export async function CityBoundary(
    ...
    try {
        ...
        const noData = (city === "无数据" || city === "沒有資料" || city === "No Data");
        if (noData) {
        ...
        if (osmId == null) {
            ...
            if (!match) {
                context.log(`CityBoundary 404: No city boundary found for city="${city}" country_code="${countryCode ?? ""}" area="${area ?? ""}" device_region="${deviceRegion ?? ""}" lat=${lat} lng=${lng}`);
            ...
            const matchedNameTag = match.matchedName ? ` matched="${match.matchedName}"` : "";
            context.log(`Geoapify resolved${mismatchTag}: "${city}" → R${osmId} "${match.name}" admin_level=${match.adminLevel} (${match.matchedBy}${matchedNameTag}${countryCode ? `, ${countryCode}` : ""})`);
