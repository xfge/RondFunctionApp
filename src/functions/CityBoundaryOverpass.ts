...
import { fetchOverpassMatch } from "../utils/overpassClient";
...
export async function CityBoundaryOverpass(
    request: HttpRequest,
    context: InvocationContext
): Promise<HttpResponseInit> {
    context.log(`CityBoundaryOverpass function processed request for url "${request.url}"`);

    try {
        const body = await parseRequestBody(request) as CityBoundaryRequest;
        context.log('CityBoundaryOverpass request body:', body);

        const validation = validateRequestBody(body, ['lat', 'lng', 'city']);
        if (!validation.isValid) {
            context.log(`CityBoundaryOverpass 400: ${validation.error}`);
        ...
        if (typeof lat !== 'number' || lat < -90 || lat > 90) {
            context.log(`CityBoundaryOverpass 400: lat must be a number between -90 and 90`);
            return createErrorResponse(400, "lat must be a number between -90 and 90");
        }
        if (typeof lng !== 'number' || lng < -180 || lng > 180) {
            context.log(`CityBoundaryOverpass 400: lng must be a number between -180 and 180`);
            return createErrorResponse(400, "lng must be a number between -180 and 180");
        }

        const noData = (city === "无数据" || city === "No Data");
        if (noData) {
            context.log(`CityBoundaryOverpass 400: No valid city data provided`);
        ...
        // If no pre-resolved osmId, call Overpass
        if (osmId == null) {
            const match = await fetchOverpassMatch(lat, lng, city, countryCode, area, (...args) => context.log(...args));
            if (!match) {
                context.log(`CityBoundaryOverpass 404: No city boundary found for the given coordinates and city name`);
                return createErrorResponse(404, "No city boundary found for the given coordinates and city name");
            }
            osmId = match.osmId;
            context.log(`Overpass resolved: "${city}" → R${osmId} "${match.name}" (${match.matchedBy})`);
            ...
            if (route.source === "amap") {
                ...
                if (!amapName) {
                    context.log(`CityBoundaryOverpass 404: No Chinese name available from Overpass for R${osmId}`);
                    return createErrorResponse(404, `No Chinese name available from Overpass for R${osmId}`);
                }
            }
        } else {
        ...
        if (route.source === "amap") {
            if (!amapName) {
                context.log(`CityBoundaryOverpass 404: No Chinese name available for R${osmId}`);
        ...
        context.log(`Error in CityBoundaryOverpass: ${error.message}`);
...
async function handleOSMBoundary(
    ...
    if (!geojson) {
        context.log(`CityBoundaryOverpass 404: Boundary geometry not available for OSM relation R${osmId}`);
...
async function handleAmapBoundary(
    ...
    if (!geojson) {
        context.log(`CityBoundaryOverpass 404: AMap boundary not available for "${chineseName}"${fallbackName ? ` / "${fallbackName}"` : ""} (R${osmId})`);
...
app.http('CityBoundaryOverpass', {
    methods: ['POST'],
    authLevel: 'anonymous',
    handler: CityBoundaryOverpass,
});
